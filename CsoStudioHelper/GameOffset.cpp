...
BOOL GameOffset::FindGameAddres()
{
	...
	for (ULONG Section = 0,Offset = 0, Count = 0; m_Memory->GetScanCodeAddress(xorstr_("CC A1 ?? ?? ?? ?? C3 CC"), m_GameAddres.dwPlayerSequence, m_pClient->hModule, Offset, Section); Count++)
	{
		if (Count == 3)
		{
	...
	*/
	...
	if (m_Memory->GetScanCode(xorstr_("68 ?? ?? ?? ?? FF 35 ?? ?? ?? ?? 89 7D E8"), m_GameAddres.dwItemPoint, m_pEngine->hModule))
	{
		/*
